...
class Spoilt extends CI_Controller {
...
$data = array('title' => $page, "items"=>$result,"mess"=>'<p class="alert alert-danger"> Fill all details and submit again</p>');
		...
		$result= $this->Spoilt_model->get_contents();
			...
			if ($this->Spoilt_model->SaveForm($form_data) == TRUE) // the information has therefore been successfully saved in the db
			{
				$page="Add Spoilt";
						  
$data = array('title' => $page, "items"=>$result,"mess"=>'<p class="alert alert-success"> Data saved successfully</p>');
...
$data = array('title' => $page, "items"=>$result,"mess"=>'<p class="alert alert-danger"> Error. Did not save, submit again</p>');
